...
import { ArrowLeftRight, Check, ChevronDown, PanelLeft } from "lucide-react";
...
export function DashboardTopbar() {
  ...
  /**
   * Flips the currently active language pair so source and target languages
   * are swapped, then persists the reversed pair through the shared context.
   */
  const handleFlipPair = useCallback(() => {
    const flippedPair = createLanguagePair(
      activePair.targetLanguage,
      activePair.sourceLanguage,
    );
    setActivePair(flippedPair);
  ...
  return (
    <div className="flex flex-col gap-4 rounded-[2rem] border border-line bg-white/84 px-5 py-4 shadow-[0_12px_30px_rgba(18,18,18,0.04)] lg:flex-row lg:items-center lg:justify-between">
      ...
      <div className="flex w-full items-center gap-3 lg:max-w-[28rem]">
        ...
        <button
          ...
          aria-label="Flip source and target languages"
          title="Flip source and target languages"
          className="shrink-0 inline-flex h-[3.4rem] w-[3.4rem] items-center justify-center rounded-[1.4rem] border border-[#ececf3] bg-[#fafafe] text-slate-700 transition hover:border-line hover:bg-white hover:text-brand focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand"
        >
